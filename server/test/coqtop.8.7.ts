...
describe("Coqtop 8.6", function() {
  before("check if coqtop exists", function() {
    if (
      !fs.existsSync(path.join(COQBIN_8_6, "/coqtop")) &&
      (os.platform() !== "win32" ||
        !fs.existsSync(path.join(COQBIN_8_6, "/coqtop.exe")))
    ) {
      console.warn("Cannot find coqtop: " + coqtopBin());
      console.warn(
        "Please make sure you have set env-var COQBIN_8_6 to point to the binaries directory of Coq 8.6."
      );
      this.skip();
    }
  });
  ...
  describe("Commands", function() {
    ...
    it("Add", async function() {
      ...
      const result = await coq.coqAddCommand(
        "Check nat.",
        1,
        currentState,
        true
    ...
    it("Add 'Check', Goal", async function() {
      ...
      const notice = messages.find(
        x => x.level === coqProto.MessageLevel.Notice
      );
      ...
      assert.deepStrictEqual(notice, {
        ...
        message: {
          scope: "_",
          text: [
            { scope: "constr.reference", text: "nat" },
            "\n     : ",
