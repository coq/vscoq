...
function getText(text: string, range?: vscode.Range): string {
  ...
  if (range.start.line === range.end.line)
    return lineAt(range.start.line).substring(
      range.start.character,
      range.end.character
    );
  else
    return (
      lineAt(range.start.line).substring(range.start.character) +
      lines
        .slice(range.start.line + 1, range.end.line)
        .map((l, idx) => l + newLines[idx + range.start.line + 1])
        .join("") +
...
class MockCoqTop extends coqtop.CoqTop {
  ...
  getVersion() {
    return semver.coerce("mock-version");
  }
  async startCoq() {
    return { stateId: 0 };
  }
  ...
  async coqQuit() {}
  async coqGoal(): Promise<{ mode: "no-proof" }> {
    return { mode: "no-proof" };
  }
  async getStatus(force: boolean) {
    return { path: [], allProofs: [], proofNumber: 0 };
  }
  async coqAddCommand(
    command: string,
    editId: number,
    ...
    return { stateId: ++this.currentState, message: "" };
  }
  async coqEditAt(stateId: number) {
    return {};
  }
  async coqLtacProfilingResults(stateId?: number, routeId?: number) {}
  async coqResizeWindow(columns: number) {}
  async coqQuery(query: string, stateId?: number, routeId?: number) {
    return;
  }
...
describe("CoqStateMachine", function() {
  ...
  describe("applyChangesToDocumentText", function() {
    it("STM.applyChangesToDocumentText", async function() {
      ...
      assert.ok(
        stm.assertSentenceConsistency(),
        "Sentence states are inconsistent"
      );
    });
  });
