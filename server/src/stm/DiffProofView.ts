...
function diffHypotheses(oldHyps: Hypothesis[], newHyps: Hypothesis[]) : Hypothesis[] {
  ...
  newHyps.forEach((hyp,idxHyp) => {
    ...
    if(oldHyp === undefined)
      results.push({diff: HypothesisDifference.New, expression: hyp.expression, identifier: hyp.identifier, relation: hyp.relation})
    else {
      const diff = text.diffText(oldHyp.expression,hyp.expression);
      results.push({
        diff: diff.different ? HypothesisDifference.Changed : HypothesisDifference.None,
