...
export type CommandIterator = (
  begin: Position,
  end?: Position
) => Iterable<{ text: string; range: Range }>;

type GoalErrorResult =
  | proto.NotRunningResult
  | proto.FailureResult
...
export class CoqStateMachine {
  ...
  private startFreshCoqtop() {
    ...
    this.coqtop.onMessage((x1, routeId, stateId) =>
      this.onCoqMessage(x1, routeId, stateId)
    );
    this.coqtop.onClosed((isError: boolean, message?: string) =>
      this.onCoqClosed(isError, message)
    );
  }

  public dispose() {
    if (this.status === STMStatus.Shutdown) return; // already disposed
    ...
    if (this.coqtop) this.coqtop.dispose();
  }
  ...
  public async interrupt(): Promise<void> {
    if (!this.isBusy() || this.isShutdown()) return;
    else this.status = STMStatus.Interrupting;
    try {
      await this.coqtop.coqInterrupt();
    } catch (err) {
      // this will fail on user interrupt
      this.console.error(
        "Exception thrown while interrupting Coq: " + err.toString()
      );
    } finally {
  ...
  public getFocusedPosition(): Position {
    if (!this.focusedSentence) return Position.create(0, 0);
  ...
  public getStatesText(): string {
    ...
    const texts: string[] = [];
    for (let sent of this.root.descendants()) texts.push(sent.getText());
    return texts.join("");
  }
   ...
   */
  private applyChangesToSentences(
    sortedChanges: TextDocumentContentChangeEvent[],
    updatedDocumentText: string
  ): State[] {
    const invalidatedSentences: State[] = [];
    try {
      const deltas = sortedChanges.map(c =>
        textUtil.toRangeDelta(c.range, c.text)
      );

      if (this.currentError && this.currentError.range) {
        for (let idx = 0; idx < sortedChanges.length; ++idx) {
          this.currentError.range = textUtil.rangeDeltaTranslate(
            this.currentError.range,
            deltas[idx]
          );
        }
      }
      ...
      for (let sent of this.lastSentence.backwards()) {
        ...
        // apply the changes
        const preserved = sent.applyTextChanges(
          sortedChanges,
          deltas,
          updatedDocumentText
        );
   ...
   */
  private async cancelInvalidatedSentences(
    invalidatedSentences: State[]
  ): Promise<void> {
    ...
    if (invalidatedSentences.length <= 0) return;
    // Cancel the invalidated sentences
    const releaseLock = await this.editLock.lock();
    try {
      if (this.status === STMStatus.Busy) await this.interrupt();
      ...
      this.focusSentence(
        invalidatedSentences[invalidatedSentences.length - 1].getParent()
      );
    } catch (err) {
      this.handleInconsistentState(err);
    } finally {
  ...
  public applyChanges(
    ...
    if (!this.isRunning() || sortedChanges.length == 0) return true;

    const invalidatedSentences = this.applyChangesToSentences(
      sortedChanges,
      updatedDocumentText
    );
  ...
  public async stepForward(
    commandSequence: CommandIterator,
    verbose: boolean = false
  ): Promise<
    (proto.FailureResult & proto.FocusPosition) | proto.NotRunningResult | null
  > {
    const endCommand = await this.startCommand();
    if (!endCommand) return;
    try {
      ...
      await this.iterateAdvanceFocus({
        iterateCondition: command =>
          textUtil.positionIsEqual(command.range.start, currentFocus),
  ...
  private convertCoqTopError(err): GoalErrorResult {
    if (err instanceof coqtop.Interrupted)
      return {
        type: "interrupted",
        range: this.sentences.get(err.stateId).getRange()
      };
  ...
  public async getGoal(): Promise<GoalResult> {
    ...
    const endCommand = await this.startCommand();
    if (!endCommand) return { type: "busy" };
    try {
      ...
      if (error instanceof coqtop.CallFailure) {
        ...
        const failure = await this.handleCallFailure(error, {
          range: sent.getRange(),
          text: sent.getText()
        });
        return Object.assign(failure, <proto.FailureTag>{ type: "failure" });
      } else return this.convertCoqTopError(error);
    } finally {
      endCommand();
    }
  }
  ...
  public async getCachedGoal(
    ...
    try {
      const state =
        (direction === "subsequent" ? this.getStateAt(pos) : null) ||
        this.getPrecedingStateAt(pos);
      if (state && state.hasGoal())
        return Object.assign(
          { type: "proof-view" } as { type: "proof-view" },
          state.getGoal(this.goalsCache)
        );
      else return { type: "no-proof" };
    } catch (error) {
  ...
  private getPrecedingStateAt(pos: vscode.Position): State | null {
    ...
    for (let s of this.sentences.values()) {
      if (
        textUtil.positionIsBeforeOrEqual(s.getRange().end, pos) &&
        textUtil.positionIsAfter(s.getRange().start, preceding.getRange().start)
      )
        preceding = s;
      if (s.contains(pos)) return s.getParent();
    }
    return preceding;
  }
  ...
  > {
    ...
    try {
      ...
      await this.iterateAdvanceFocus({
        iterateCondition: command => {
          return (
            ((!interpretToEndOfSentence &&
              textUtil.positionIsAfterOrEqual(position, command.range.end)) ||
              (interpretToEndOfSentence &&
                textUtil.positionIsAfter(position, command.range.start))) &&
      ...
      if (token && token.isCancellationRequested) throw "operation interrupted";

      if (
        !interpretToEndOfSentence &&
        textUtil.positionIsBefore(position, this.getFocusedPosition())
      ) {
        // We exceeded the desired position
        await this.focusSentence(this.getParentSentence(position));
      } else if (
        interpretToEndOfSentence &&
        textUtil.positionIsBeforeOrEqual(
          position,
          this.focusedSentence.getRange().start
        )
      ) {
  ...
  public async doQuery(
    ...
    if (!this.isCoqReady()) return;
    const endCommand = await this.startCommand();
    if (!endCommand) return;
    try {
      let state: StateId = undefined;
      state = position
        ? this.getParentSentence(position).getStateId()
        : this.focusedSentence.getStateId();
      await this.refreshOptions();
  ...
  public async setDisplayOptions(
    options: { item: proto.DisplayOption; value: proto.SetDisplayOption }[]
  ) {
    function set(old: boolean, change: proto.SetDisplayOption): boolean {
      switch (change) {
        case proto.SetDisplayOption.On:
          return true;
        case proto.SetDisplayOption.Off:
          return false;
        case proto.SetDisplayOption.Toggle:
          return !old;
      }
    }
    for (let option of options) {
      switch (option.item) {
        case proto.DisplayOption.AllLowLevelContents: {
          ...
          const value = set(
            this.currentCoqOptions.printingAll &&
              this.currentCoqOptions.printingExistentialInstances &&
              this.currentCoqOptions.printingUniverses,
            option.value
          );
        ...
        case proto.DisplayOption.AllBasicLowLevelContents:
          this.currentCoqOptions.printingAll = set(
            this.currentCoqOptions.printingAll,
            option.value
          );
          break;
        case proto.DisplayOption.Coercions:
          this.currentCoqOptions.printingCoercions = set(
            this.currentCoqOptions.printingCoercions,
            option.value
          );
          break;
        case proto.DisplayOption.ExistentialVariableInstances:
          this.currentCoqOptions.printingExistentialInstances = set(
            this.currentCoqOptions.printingExistentialInstances,
            option.value
          );
          break;
        case proto.DisplayOption.ImplicitArguments:
          this.currentCoqOptions.printingImplicit = set(
            this.currentCoqOptions.printingImplicit,
            option.value
          );
          break;
        case proto.DisplayOption.Notations:
          this.currentCoqOptions.printingNotations = set(
            this.currentCoqOptions.printingNotations,
            option.value
          );
          break;
        case proto.DisplayOption.RawMatchingExpressions:
          this.currentCoqOptions.printingMatching = set(
            this.currentCoqOptions.printingMatching,
            option.value
          );
          break;
        case proto.DisplayOption.UniverseLevels:
          this.currentCoqOptions.printingUniverses = set(
            this.currentCoqOptions.printingUniverses,
            option.value
          );
          break;
  ...
  public *getSentences(): IterableIterator<{
    range: Range;
    status: StateStatus;
  }> {
    if (!this.isRunning()) return;
  ...
  public *getDiagnostics(): IterableIterator<CoqDiagnostic> {
    ...
    if (this.currentError !== null)
      yield Object.assign(this.currentError, <CoqDiagnostic>{
  ...
  private getParentSentence(position: Position): State {
    for (let sentence of this.root.descendants()) {
      if (!sentence.isBefore(position)) return sentence.getParent();
    }
  ...
  private getSentence(position: Position): State {
    for (let sentence of this.root.descendants()) {
      if (sentence.contains(position)) return sentence;
    }
    // This should never be reached
    return this.root;
  }
  ...
  private initialize(rootStateId: StateId) {
    if (this.root != null) throw "STM is already initialized.";
    if (this.isShutdown())
  ...
  private async validateState(initialize: boolean): Promise<boolean> {
    ...
    else if (this.isShutdown()) return false;
    else if (this.isCoqReady()) return true;
    else if (initialize) {
      if (!this.coqtop) this.startFreshCoqtop();
   ...
   * */
  private async iterateAdvanceFocus(params: {
    iterateCondition: (
      command: { text: string; range: Range },
      contiguousFocus: boolean
    ) => boolean;
    commandSequence: CommandIterator;
    verbose: boolean;
    end?: Position;
    ...
    if (params.synchronous === undefined) params.synchronous = false;

    // true if the focus has not jumped elsewhere in the document
    let contiguousFocus = true;
    // Start advancing sentences
    let commandIterator = params
      .commandSequence(this.getFocusedPosition(), params.end)
      [Symbol.iterator]();
    for (let nextCommand = commandIterator.next(); !nextCommand.done; ) {
      ...
      if (!params.iterateCondition(command, contiguousFocus)) return;
  ...
  private async addCommand(
    command: { text: string; range: Range },
    verbose: boolean
  ): Promise<{ sentence: State; unfocused: boolean }> {
    try {
      ...
      if (!textUtil.positionIsEqual(parent.getRange().end, command.range.start))
        this.throwInconsistentState(
          "Can only add a comand to the current focus"
        );

      const value = await this.coqtop.coqAddCommand(
        command.text,
        this.version,
        parent.getStateId(),
        verbose
      );
      const newSentence = State.add(
        parent,
        command.text,
        value.stateId,
        command.range,
      ...
      if (
        textUtil.positionIsAfterOrEqual(
          newSentence.getRange().start,
          this.lastSentence.getRange().end
        )
      )
      ...
      if (typeof error === "string") throw new InconsistentState(error);
      else if (error instanceof coqtop.CallFailure) {
        const failure = await this.handleCallFailure(error, {
          range: command.range,
          text: command.text
        });
        throw new AddCommandFailure(
          failure.message,
          failure.range,
          failure.range
        );
  ...
  private async handleCallFailure(
    error: coqtop.CallFailure,
    command: { range: Range; text: string }
  ): Promise<proto.FailValue> {
    let errorRange: Range = undefined;
    if (error.range)
      errorRange = vscode.Range.create(
        textUtil.positionAtRelativeCNL(
          command.range.start,
          command.text,
          error.range.start
        ),
    ...
    return <proto.Goal>{
      ...
      hypotheses: goal.hypotheses.map(hyp => {
        ...
        const result = {
          ...
          expression: text.normalizeText(
            server.project.getPrettifySymbols().prettify(h.rest)
          )
        };
        return result;
      })
  ...
  private convertUnfocusedGoals(
    focusStack: coqProto.UnfocusedGoalStack
  ): proto.UnfocusedGoalStack {
    if (focusStack)
      return {
    ...
    switch (goals.mode) {
      ...
      case "proof":
        const pv = this.goalsCache.cacheProofView({
          ...
          abandonedGoals: (goals.abandonedGoals || []).map(
            this.parseConvertGoal
          ),
        ...
        return {
          type: "proof-view",
          ...this.focusedSentence.getGoal(this.goalsCache)
        };
      default:
        this.console.warn(
          "Goal returned an unexpected value: " +
            util.inspect(goals, false, undefined)
        );
    }
  }
  ...
  private handleInconsistentState(error: any) {
    this.callbacks.coqDied(
      proto.CoqtopStopReason.InternalError,
      "Inconsistent state: " + error.toString()
    );
    this.dispose();
  }
  ...
  private async focusSentence(sentence?: State) {
    if (
      !sentence ||
      sentence == this.focusedSentence ||
      !this.sentences.has(sentence.getStateId())
    )
      return;
    try {
      ...
      if (error.stateId) await this.gotoErrorFallbackState(error.stateId);
    }
  }
  ...
  private rewindTo(newLast: State) {
    for (let sent of newLast.descendants()) this.deleteSentence(sent);
    newLast.truncate();
  ...
  private applyBufferedFeedback() {
    ...
    this.bufferedFeedback.forEach(feedback => {
      const sent = this.sentences.get(feedback.stateId);
      if (!sent) {
        this.console.warn(
          "Received buffered feedback for unknown stateId: " + feedback.stateId
        );
        return;
      }
  ...
  private setFocusedSentence(sent?: State) {
    if (sent === this.focusedSentence) return;
    this.focusedSentence = sent;
    this.callbacks.updateStmFocus(this.getFocusedPosition());
  }

  private onCoqMessage(
    msg: coqProto.Message,
    routeId: coqProto.RouteId,
    stateId?: StateId
  ) {
    const prettyMessage = text.normalizeText(
      server.project
        .getPrettifySymbols()
        .prettify(errorParsing.parseError(msg.message))
    );
    if (msg.level === coqProto.MessageLevel.Error && stateId !== undefined) {
      const sent = this.sentences.get(stateId);
      if (sent) {
        var range: Range = sent.pushDiagnostic(
          prettyMessage,
        ...
        this.console.warn(
          `Error for unknown stateId: ${stateId}; message: ${msg.message}`
        );
      }
    } else if (
      msg.level === coqProto.MessageLevel.Warning &&
      stateId !== undefined
    ) {
      const sent = this.sentences.get(stateId);
      if (sent) {
        ...
        this.console.warn(
          `Warning for unknown stateId: ${stateId}; message: ${msg.message}`
        );
      }
    } else this.callbacks.message(msg.level, prettyMessage, routeId);
  }

  private onFeedback(feedback: coqProto.StateFeedback) {
    const hasStateId = feedback.objectId.objectKind === "stateid";
    const stateId = coqProto.hasStateId(feedback.objectId)
      ? feedback.objectId.stateId
      : undefined;
  ...
  private async onCoqClosed(isError: boolean, message?: string) {
    this.callbacks.coqDied(
      isError
        ? proto.CoqtopStopReason.Anomaly
        : proto.CoqtopStopReason.UserRequest,
      message
    );
  ...
  private debuggingGetSentences(params?: {
    begin?: State | string;
    end?: State | string;
  }) {
    let begin: State, end: State;
    if (params && params.begin === "focus") begin = this.focusedSentence;
    if (!params || !params.begin || typeof params.begin === "string")
      begin = this.root;
    else begin = <State>params.begin;

    if (params && params.end === "focus") end = this.focusedSentence;
    else if (!params || !params.end || typeof params.end === "string")
      end = this.lastSentence;
    else end = <State>params.end;
  ...
  public assertSentenceConsistency(): boolean {
    ...
    while (prev.getNext()) {
      ...
      if (textUtil.positionIsBefore(range.start, prev.getRange().end))
        error(
          `States are out of order: id=${prev.getStateId()} and id=${st.getStateId()}`
        );
    ...
    for (let id of this.sentences.keys()) {
      if (!ids.has(id)) error(`State is mapped but unreachable: id=${id}`);
    }
  ...
  public logDebuggingSentences(ds?: DSentence[]) {
    if (!ds) ds = this.debuggingGetSentences();
    this.console.log(
      ds.map((s, idx) => "  " + (1 + idx) + ":\t" + s).join("\n")
    );
  }
}
...
function abbrString(s: string) {
  ...
  if (s2.length > 80) return s2.substr(0, 80 - 3) + "...";
  else return s2;
}

type DSentence = string;
function createDebuggingSentence(sent: State): DSentence {
  return `${sent.getRange().start.line}:${sent.getRange().start.character}-${
    sent.getRange().end.line
  }:${sent.getRange().end.character} -- ${abbrString(sent.getText().trim())}`;
}
