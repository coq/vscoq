...
export function copyAnnotation(x: Annotation): Annotation {
  ...
  if (x.diff !== undefined) return { diff: x.diff };
  if (x.substitution !== undefined) return { substitution: x.substitution };
  else return {};
}
...
export function textSplit(
  ...
  limit?: number
): { splits: (string | TextAnnotation | ScopedText)[]; rest: AnnotatedText } {
  if (typeof text === "string") {
    const result = text.split(separator as any).filter(v => v !== "");
    return {
      splits: result.slice(0, limit),
      rest: limit === undefined ? [] : result.slice(limit)
    };
  } else if (text instanceof Array) {
    ...
    for (
      ;
      /**/ (limit === undefined || count < limit) && idx < text.length;
      ++idx
    ) {
      const { splits: splits2, rest: rest2 } = textSplit(
        text[idx],
        separator,
        limit === undefined ? undefined : limit - count
      );
      splits.push(...splits2);
      count += splits2.length;
      rest = rest2;
    }
    if (limit === undefined) return { splits: splits, rest: [] };
    else {
      if (rest instanceof Array)
        return {
          splits: splits.slice(0, limit),
          rest: [...splits.slice(limit), ...rest, ...text.slice(idx)]
        };
      else
        return {
          splits: splits.slice(0, limit),
    ...
    const { splits: splits, rest: rest } = textSplit(
      text.text,
      separator,
      limit
    );
    const splitsResult = splits.map((s: AnnotatedText) =>
      text.attributes
        ? { scope: text.scope, attributes: text.attributes, text: s }
        : { scope: text.scope, text: s }
    );
    const restResult: ScopedText = { scope: text.scope, text: rest };
    if (text.attributes) restResult.attributes = text.attributes;
...
function subtextHelper<T extends AnnotatedText>(
  text: T,
  pos: number,
  start: number,
  end?: number
): { result: T; length: number; next: number } {
  if (typeof text === "string") {
    const t = text as string;
    const s = t.substring(
      start - pos,
      end === undefined ? undefined : end - pos
    );
    ...
    for (
      let idx = 0;
      idx < text.length && (end === undefined || pos < end);
      ++idx
    ) {
      const s = subtextHelper<string | TextAnnotation | ScopedText>(
        text[idx],
        pos,
        start,
        end
      ...
      if (s.length > 0) {
        if (s.result instanceof Array) strs.push(...s.result);
        else strs.push(s.result);
      }
      len += s.length;
      pos = s.next;
    }
...
function mapAnnotationInternal(
  text: AnnotatedText,
  map: (
    text: string,
  ...
  currentDisplayOffset: number
): { text: AnnotatedText; endOffset: number; endDisplayOffset: number } {
  if (typeof text === "string") {
    const result = map(text, {}, currentOffset, currentDisplayOffset);
    return {
      text: result,
      endOffset: currentOffset + textLength(result),
      endDisplayOffset: currentDisplayOffset + textDisplayLength(result)
    };
  } else if (text instanceof Array) {
    const results: (string | TextAnnotation | ScopedText)[] = [];
    for (let txt of text) {
      const newText = mapAnnotationInternal(
        txt,
        map,
        currentOffset,
      ...
      if (newText.text instanceof Array) results.push(...newText.text);
      else results.push(newText.text);
    }
    return {
      text: results,
      endOffset: currentOffset,
      endDisplayOffset: currentDisplayOffset
    };
  } else if (isScopedText(text)) {
    const result = mapAnnotationInternal(
      text.text,
      map,
      currentOffset,
      currentDisplayOffset
    ...
    if (text.attributes) resultText.attributes = text.attributes;
    return {
      text: resultText,
      endOffset: result.endOffset,
      endDisplayOffset: result.endDisplayOffset
    };
  } else {
    // TextAnnotation
    const newText = map(
    ...
    return {
      text: newText,
      endOffset: currentOffset + textLength(newText),
      endDisplayOffset: currentDisplayOffset + textDisplayLength(newText)
    };
  }
}
 ...
 */
export function compatibleAnnotations<T extends Annotation>(
  ann1: T,
  ann2: T
): boolean {
  return (
...
function concatText(text1: AnnotatedText, text2: AnnotatedText): AnnotatedText {
  if (typeof text1 === "string" && typeof text2 === "string")
    return [text1 + text2];
  else if (text1 instanceof Array) return text1.concat(text2);
  else if (text2 instanceof Array) return [text1, ...text2];
  else return [text1, text2];
}

export function tryCombineText(
  text1: string | TextAnnotation | ScopedText,
  text2: string | TextAnnotation | ScopedText
): string | TextAnnotation | ScopedText | undefined {
  if (typeof text1 === "string" && typeof text2 === "string")
    return text1 + text2;
  else if (text1 === "") return text2;
  else if (text2 === "") return text1;
  else if (
    isScopedText(text1) &&
    isScopedText(text2) &&
    text1.scope === text2.scope &&
    text1.attributes === text2.attributes
  ) {
    if (text1.attributes)
      return {
        scope: text1.scope,
        attributes: text1.attributes,
        text: normalizeText(concatText(text1.text, text2.text))
      };
    else
      return {
        scope: text1.scope,
    ...
    isTextAnnotation(text1) &&
    isTextAnnotation(text2) &&
    compatibleAnnotations(text1, text2)
  ) {
    return normalizeTextAnnotationOrString({
      diff: text1.diff,
      substitution: text1.substitution || text2.substitution,
...
export function normalizeTextAnnotationOrString(
  text: TextAnnotation
): TextAnnotation | string;
export function normalizeTextAnnotationOrString(
  text: TextAnnotation | string
): TextAnnotation | string {
  if (typeof text === "string") {
    return text;
  } else {
    ...
    let count = 0;
    for (let key in text) {
      if (text.hasOwnProperty(key) && text[key] !== undefined) ++count;
    }
    for (let key in text) {
      if (text.hasOwnProperty(key) && text[key] === undefined) delete text[key];
    }
    if (count > 1)
      // has annotations
      return text;
...
export function normalizeText(text: AnnotatedText): AnnotatedText {
  if (typeof text === "string") {
    ...
    if (norm instanceof Array) results.push(...norm);
    else results.push(norm);
    ...
    for (let idx = 1; idx < text.length; ++idx) {
      ...
      if (norm instanceof Array) {
        ...
        if (merge) results[results.length - 1] = merge;
        else results.push(merge);
        results.push(...norm);
      } else {
...
export function combineAnnotationText(
  text: TextAnnotation | string,
  baseAnn: Annotation
): TextAnnotation | string {
  ...
  else result = Object.assign(copyAnnotation(baseAnn), text);
...
function toDiff(
  ...
  else if (mode === "old" && d.removed) return "removed";
  else return undefined;
}
...
export function annotateDiffAdded(
  ...
  const result = mapAnnotation(
    text,
    (plainText, annotation, startOffset, startDisplayOffset) => {
      let text: string;
      let start: number;
      let doSubst = false;
      if (options.diffSubstitutions) start = startDisplayOffset;
      ...
      for (
        ;
        idx < differences.length &&
        offset + differences[idx].value.length <= start + text.length;
        ++idx
      ) {
        ...
        if (doSubst) {
          ...
          parts.push(
            combineAnnotationText(
              {
                text: diff.value.substr(diffOffset),
                diff: toDiff(diff, options.mode)
              },
              annotation
            )
        ...
        if (annotation.substitution) annotation.substitution = "";
      ...
      if (idx < differences.length && offset < start + text.length) {
        ...
        if (doSubst) {
          ...
          parts.push(
            combineAnnotationText(
              {
                text: text.substring(offset + diffOffset - start),
                diff: toDiff(differences[idx], options.mode)
              },
              annotation
            )
...
export function diffText(
  oldText: AnnotatedText,
  newText: AnnotatedText,
  normalize = true
): { text: AnnotatedText; different: boolean } {
  if (!oldText) return { text: newText, different: false };
  const difference = diff.diffWords(
    textToDisplayString(oldText),
    textToDisplayString(newText)
  );
  const result = annotateDiffAdded(newText, difference, {
    diffSubstitutions: true,
    mode: "new"
  });
  if (normalize)
    return { text: normalizeText(result), different: difference.length > 1 };
  else return { text: result, different: difference.length > 1 };
}

export function append(...texts: AnnotatedText[]): AnnotatedText {
  const results: (string | TextAnnotation | ScopedText)[] = [];
  for (let txt of texts) {
    if (txt instanceof Array) results.push(...txt);
    else results.push(txt);
  }
