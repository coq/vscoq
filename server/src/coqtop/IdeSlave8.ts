...
export class IdeSlave extends coqtop.IdeSlave {
  ...
  private coqResultValueListener: {
    onValue: (value: coqProto.ValueReturn | coqProto.FailValue) => void;
  ...
  protected connect(
    version: string,
    mainR: NodeJS.ReadableStream,
    mainW: NodeJS.WritableStream,
    controlR: NodeJS.ReadableStream,
    controlW: NodeJS.WritableStream
  ) {
    ...
    this.parser = new coqXml.XmlStream(this.mainChannelR, deserializer, {
      onFeedback: (feedback: coqProto.StateFeedback) =>
        this.doOnFeedback(feedback),
      onMessage: (
        msg: coqProto.Message,
        routeId: coqProto.RouteId,
        stateId?: coqProto.StateId
      ) => this.doOnMessage(msg, routeId, stateId),
      onOther: (tag: string, x: any) => this.doOnOther(tag, x),
      onError: (x: any) => this.doOnSerializationError(x)
    });
    this.parser.on("error", err => {
      if (this.coqResultValueListener) this.coqResultValueListener.onError(err);
    });
    ...
    if ((mainR as any) == (mainW as any)) {
      this.setupChannel(mainR, "main channel", data =>
        this.onMainChannelR(data)
      );
    } else {
      this.setupChannel(mainR, "main channel (R)", data =>
        this.onMainChannelR(data)
      );
    ...
    if ((controlR as any) === (controlW as any)) {
      this.setupChannel(controlR, "control channel", data =>
        this.onControlChannelR(data)
      );
    } else {
      this.setupChannel(controlR, "control channel (R)", data =>
        this.onControlChannelR(data)
      );
  ...
  private setupChannel(
    ...
    if (dataHandler) channel.on("data", (data: string) => dataHandler(data));
  ...
  public dispose() {
    ...
    if (this.mainChannelW) this.mainChannelW.end();
    if (this.controlChannelW) this.controlChannelW.end();
  ...
  protected onCoqTopError(
    error: string | { message: string },
    channelName: string
  ): void {
    try {
      const message = typeof error === "string" ? error : error.message;
      if (this.state !== IdeSlaveState.Connected) return;
  ...
  private doOnFeedback(feedback: coqProto.StateFeedback) {
    if (this.callbacks.onFeedback) this.callbacks.onFeedback(feedback);
  }
  ...
  private validateValue(value: coqProto.FailValue, logIdent?: string): never {
    if (value.message === "User interrupt.")
      ...
      if (value.location) error.range = value.location;
      throw error;
    }
  }
  ...
  private coqGetResultOnce(logIdent?: string): Promise<coqProto.ValueReturn> {
    if (this.coqResultValueListener)
      new InternalError(
        "Multiple handlers are being registered for the same response value from Coqtop."
      );
    return new Promise<coqProto.ValueReturn>((resolve, reject) => {
      this.coqResultValueListener = {
        onValue: (value: coqProto.ValueReturn | coqProto.FailValue) => {
          ...
          try {
            if (value.status === "good") resolve(value);
            else this.validateValue(value, logIdent);
          } catch (error) {
            reject(error);
          }
        },
  ...
  protected async checkState(): Promise<void> {
    if (!this.isConnected()) throw new IdeSlaveNotConnectedError();
  }
  ...
  public async coqQuit(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        else this.console.log(`Forced Quit (timeout).`);
      }
    } catch (error) {
  ...
  public async getStatus(force: boolean): Promise<coqProto.CoqStatus> {
    ...
    this.writeMain(
      `<call val="Status"><bool val="${force ? "true" : "false"}" /></call>`
    );

    return coqProto.GetValue("Status", await coqResult);
  }

  public async coqAddCommand(
    command: string,
    editId: number,
    stateId: number,
    verbose?: boolean
    ...
    this.console.log("--------------------------------");
    this.console.log(
      `Call Add("${command.trim().substr(0, 20) +
        (command.trim().length > 20
          ? "..."
          : "")}", editId: ${editId}, stateId: ${stateId}, verbose: ${verboseStr})`
    );
    this.writeMain(
      `<call val="Add"><pair><pair><string>${coqXml.escapeXml(
        command
      )}</string><int>${editId}</int></pair><pair><state_id val="${stateId}"/><bool val="${verboseStr}"/></pair></pair></call>`
    );
    ...
    if (result.stateId === undefined)
      this.console.log(
        `UNDEFINED Add: ` + util.inspect(value, false, undefined)
      );
    this.console.log(
      `Add:  ${stateId} --> ${result.stateId} ${
        result.unfocusedStateId ? `(unfocus ${result.unfocusedStateId})` : ""
      } "${result.message || ""}"`
    );
    return result;
  }
  ...
  public async coqEditAt(stateId: number): Promise<EditAtResult> {
    ...
    this.console.log(
      `EditAt: ${stateId} --> ${
        result.enterFocus
          ? `{newTipId: ${result.enterFocus.stateId}, qedId: ${result.enterFocus.qedStateId}, oldId: ${result.enterFocus.oldStateIdTip}}`
          : "{}"
      }`
    );
    return result;
  ...
  public async coqLtacProfilingResults(
    stateId?: number,
    routeId?: number
  ): Promise<void> {
    await this.checkState();
    ...
    this.console.log("--------------------------------");
    this.console.log(
      `Call Query(query: "Show Ltac Profile.", stateId: ${stateId}, routeId: ${routeId})`
    );
    this.writeMain(
      `<call val="Query"><pair><route_id val="${routeId}"/><pair><string>Show Ltac Profile.</string><state_id val="${stateId}"/></pair></pair></call>`
    );
  ...
  public async coqResizeWindow(columns: number): Promise<void> {
    ...
    this.writeMain(
      `<call val="SetOptions"><list><pair><list><string>Printing</string><string>Width</string></list><option_value val="intvalue"><option val="some"><int>${columns}</int></option></option_value></pair></list></call>`
    );
    coqProto.GetValue("SetOptions", await coqResult);
    this.console.log(`ResizeWindow: ${columns} --> ()`);
  }

  public async coqQuery(
    query: string,
    stateId: number,
    routeId?: number
  ): Promise<void> {
    ...
    this.console.log("--------------------------------");
    this.console.log(
      `Call Query(stateId: ${stateId}, ${
        routeId !== undefined ? "routeId: " + routeId + ", " : ""
      }query: ${query})`
    );
    this.writeMain(
      `<call val="Query"><pair><route_id val="${routeId}"/><pair><string>${coqXml.escapeXml(
        query
      )}</string><state_id val="${stateId}"/></pair></pair></call>`
    );
  ...
  public async coqSetOptions(options: coqtop.CoqOptions): Promise<void> {
    ...
    for (let optionKey in options) {
      ...
      if (rawOptionsValue !== undefined && typeof rawOptionsName === "string") {
        ...
        xmlOptions.push(
          new xmlTypes.Pair(
            optionName,
            new xmlTypes.OptionValue(rawOptionsValue)
          )
        );
      }
    }
    ...
    this.writeMain(
      `<call val="SetOptions">${xmlTypes.encode(xmlOptions)}</call>`
    );
...
const CoqOptionsMapping = {
  ...
  printingPrimitiveProjectionCompatibility:
    "Printing Primitive Projection Compatibility",
  printingPrimitiveProjectionParameters:
