...
export class CallFailure {
  ...
  public toString() {
    return (
      textToDisplayString(this.message) +
      (this.range || this.stateId
        ? "  (" +
          (this.range
            ? `offsets ${this.range.start}-${this.range.stop}`
            : this.stateId
            ? " "
            : "") +
...
export function detectVersion(
  coqtopModule: string,
  cwd: string,
  console?: { log: (string) => void; warn: (string) => void }
): Promise<string | null> {
  if (console) console.log("exec: " + coqtopModule + " -v");
...
export abstract class IdeSlave {
  ...
  public onFeedback(
    handler: (feedback: coqProto.StateFeedback) => void
  ): { dispose: () => void } {
    this.callbacks.onFeedback = handler;
    return {
      dispose: () => {
        this.callbacks.onFeedback = undefined;
      }
    };
  }
  ...
  public onClosed(handler: (isError: boolean, message?: string) => void) {
    ...
    return {
      dispose: () => {
        this.callbacks.onClosed = undefined;
      }
    };
  }
  ...
  public abstract coqLtacProfilingResults(
    stateId?: number,
    routeId?: number
  ): Promise<void>;
  public abstract coqResizeWindow(columns: number): Promise<void>;
  public abstract coqQuery(
    query: string,
    stateId?: number,
    routeId?: number
  ): Promise<void>;
