...
export namespace Nodes {
  ...
  export function optValIsStringOpt(
    opt: OptionValueNode
  ): opt is OptionValueStringOptNode {
    return opt.$.val === "stringoptvalue";
  }
  export function optValIsBool(
    opt: OptionValueNode
  ): opt is OptionValueBoolNode {
    return opt.$.val === "boolvalue";
  }
  export function optValIsString(
    opt: OptionValueNode
  ): opt is OptionValueStringNode {
...
export abstract class Deserialize {
  ...
  private doDeserialize(value: Nodes.TypedNode): CoqValue {
    switch (value.$name) {
      ...
      case "option":
        return check(
          value.$name,
          Nodes.optionIsSome(value) ? value.$children[0] : null
        );
      case "option_value": {
        if (Nodes.optValIsInt(value)) return check(value.$name, value.option);
      ...
      case "goals": {
        return check(value.$name, {
          ...
          backgroundGoals: (value.$children[1] || []).reduce<
            UnfocusedGoalStack
          >(
            (bg, v: Pair<Subgoal[], Subgoal[]>) => ({
              before: v[0],
              next: bg,
              after: v[1]
            }),
      ...
      case "feedback_content":
        ...
        return check(
          value.$name,
          this.deserializeFeedbackContent(value as Node)
        );
      case "feedback": {
        ...
        const result = Object.assign<
          coqProto.FeedbackBase,
          coqProto.FeedbackContent
        >(
          {
            objectId: objectId,
            route: +(value.$.route || "0")
          ...
          value.$children[1]
        ) /* TODO: TS 2.1 will equate A&(B|C)=(A&B)|(A&C) so this cast will not be necessary */ as coqProto.StateFeedback;
  ...
  public deserializeFeedbackContent(value: Node): FeedbackContent {
    return this.doDeserializeFeedbackContent(
      value as Nodes.FeedbackContentNode
    );
  }

  private doDeserializeFeedbackContent(
    value: Nodes.FeedbackContentNode
  ): FeedbackContent {
