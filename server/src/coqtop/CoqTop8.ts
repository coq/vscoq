...
export class CoqTop extends IdeSlave8 implements coqtop.CoqTop {
  ...
  constructor(
    settings: CoqTopSettings,
    scriptFile: string,
    projectRoot: string,
    console: vscode.RemoteConsole
  ) {
    super(console);
  ...
  public /* override */ dispose() {
    ...
    if (this.coqtopProc) {
      try {
        ...
        if (this.coqtopProc.connected) this.coqtopProc.disconnect();
      } catch (e) {}
  ...
  public async startCoq(): Promise<InitResult> {
    ...
    let coqtopVersion = await coqtop.detectVersion(
      this.coqtopBin,
      this.projectRoot,
      this.console
    );
    ...
    else this.console.warn(`Could not detect coqtop version`);

    this.coqtopVersion = semver.coerce(coqtopVersion);
    this.console.log(
      `Coqtop version parsed into semver version ${this.coqtopVersion.format()}`
    );
  ...
  protected async /* override */ checkState(): Promise<void> {
    if (this.coqtopProc === null) this.startCoq();
    super.checkState();
  }
  ...
  private acceptConnection(
    ...
    return new Promise<net.Socket>(resolve => {
      server.once("connection", (socket: net.Socket) => {
        ...
        this.console.log(
          `Client connected on ${name} (port ${socket.localPort})`
        );
  ...
  private spawnCoqTop(mainAddr: string, controlAddr: string) {
    ...
    return spawn(coqtopModule, args, {
      detached: false,
      cwd: this.projectRoot
    });
  }

  public async /* override */ coqInterrupt(): Promise<boolean> {
    if (!this.coqtopProc) return false;
