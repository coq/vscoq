...
let loop injections =
  ...
  let rec loop (todo : LspManager.event Sel.Todo.t) =
    ...
    log @@ "Main loop event ready: " ^ Pp.string_of_ppcmds (Pp.prlist LspManager.pr_event events) ^ " , " ^ string_of_int nremaining ^ " events waiting";
    let new_events = LspManager.handle_events events in
