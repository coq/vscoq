...
let%test_unit "parse.validate_errors_twice" = 
  let _, _, doc = init_and_parse_test_doc ~text:"Lemma a : True. Proof. idtac (fun x -> x). Qed." in
  ...
  let _,_,doc = validate_document doc in
  [%test_eq: int] (List.length (Document.parse_errors doc)) 1;
  let _,_,doc = validate_document doc in
  [%test_eq: int] (List.length (Document.parse_errors doc)) 1

let %test_unit "document: invalidate top" =
  let _, _, doc = init_and_parse_test_doc ~text:"Definition x := 3. Lemma foo : x = 3. Proof. reflexivity. Qed." in
  let unchanged, invalid, doc = validate_document doc in
  [%test_eq: sentence_id list] [] (Stateid.Set.elements invalid);
