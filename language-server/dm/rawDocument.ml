...
let compare_lines doc log = 
  ...
  let lines_ocaml = compute_lines text_doc in
  let i = ref 0 in
...
let create text =
  ...
  { text; lines = compute_lines text }

let text t = Lsp.Text_document.text t.text

(* TODO: DO THIS CORRECTLY IE USING UTF16 ENCODING *)
...
let apply_text_edits raw edits = 
  ...
  let new_lines = compute_lines text_doc in
