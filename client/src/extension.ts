...
vscode.Range.prototype.toString = function rangeToString(this: vscode.Range) {
  return `[${this.start.toString()},${this.end.toString()})`;
};
vscode.Position.prototype.toString = function positionToString(
...
console.log(
  `Coq Extension: process.version: ${process.version}, process.arch: ${process.arch}}`
);
...
export function activate(context: ExtensionContext) {
  ...
  regProjectCmd("interpretToPointSynchronous", () =>
    project.interpretToPoint({ synchronous: true })
  );
  regProjectCmd("interpretToEnd", project.interpretToEnd);
  regProjectCmd("interpretToEndSynchronous", () =>
    project.interpretToEnd({ synchronous: true })
  );
  ...
  regCmd("display.toggle.implicitArguments", () =>
    project.setDisplayOption(
      proto.DisplayOption.ImplicitArguments,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.coercions", () =>
    project.setDisplayOption(
      proto.DisplayOption.Coercions,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.rawMatchingExpressions", () =>
    project.setDisplayOption(
      proto.DisplayOption.RawMatchingExpressions,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.notations", () =>
    project.setDisplayOption(
      proto.DisplayOption.Notations,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.allBasicLowLevelContents", () =>
    project.setDisplayOption(
      proto.DisplayOption.AllBasicLowLevelContents,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.existentialVariableInstances", () =>
    project.setDisplayOption(
      proto.DisplayOption.ExistentialVariableInstances,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.universeLevels", () =>
    project.setDisplayOption(
      proto.DisplayOption.UniverseLevels,
      proto.SetDisplayOption.Toggle
    )
  );
  regCmd("display.toggle.allLowLevelContents", () =>
    project.setDisplayOption(
      proto.DisplayOption.AllLowLevelContents,
      proto.SetDisplayOption.Toggle
    )
  );
...
async function withDocAsync<T>(
  editor: TextEditor,
  callback: (doc: CoqDocument) => Promise<T>
): Promise<void> {
  const doc = editor
    ? project.getOrCurrent(editor.document.uri.toString()) || null
    : null;
...
async function queryStringFromPlaceholder(prompt: string, editor: TextEditor) {
  ...
  if (editor.selection.isEmpty)
    placeHolder = editor.document.getText(
...
function querySearchAbout(editor: TextEditor, edit: TextEditorEdit) {
  return withDocAsync(editor, async doc =>
    doc.query(
      "searchAbout",
      await queryStringFromPlaceholder("Search About:", editor)
    )
  );
}
...
function searchAbout(editor: TextEditor, edit: TextEditorEdit) {
  return withDocAsync(editor, async doc =>
    doc.query(
      "searchAbout",
      await queryStringFromPosition("Search About:", editor)
    )
  );
}
...
function viewProofStateAt(editor: TextEditor, edit: TextEditorEdit) {
  return withDocAsync(editor, async doc => doc.viewGoalAt(editor));
}

function viewCurrentProofState(editor: TextEditor, edit: TextEditorEdit) {
  return withDocAsync(editor, async doc => doc.viewGoalState(editor));
}
