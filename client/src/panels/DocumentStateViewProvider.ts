...
export class DocumentStateViewProvider implements TextDocumentContentProvider {
    ...
    fire() : void {
        if(this.uri !== null) {
            this._client.writeToVscoq2Channel("[DocumentStateViewProvider] Firing event");
            this._client.writeToVscoq2Channel("[DocumentStateViewProvider] Current doc uri: " + this.textDocument?.uri.toString());
    ...
    async provideTextDocumentContent(uri: Uri, token: CancellationToken): Promise<string> {
        ...
        if(this.textDocument === null) {
            this._client.writeToVscoq2Channel("[DocumentStateViewProvider] No document was set so no state can be returned !"); 
        ...
        this._client.writeToVscoq2Channel("[DocumentStateViewProvider] Getting document state for uri: " + this.textDocument.uri.toString());
