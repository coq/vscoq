...
<iframe src="${coqViewToFileUri(
      uri
    )}" seamless style="position:absolute;top:0px;left:0px;right:0px;bottom:0px;border:none;margin:0px;padding:0px;width:100%;height:100%" />
</body>`;
  }
...
export class HtmlLtacProf {
  ...
  constructor(private results: proto.LtacProfResults) {
    if (coqViewProvider === null) {
      ...
      this.docRegistration = vscode.workspace.registerTextDocumentContentProvider(
        "coq-ltacprof",
        coqViewProvider
      );
    }
  ...
  private handleClientMessage(event: {
    data: any;
    type: string;
    target: WebSocket;
  }) {}
