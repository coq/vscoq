...
namespace DisplayOptionPicks {
  ...
  export const ImplicitArguments: T = {
    ...
    description: "toggle display of *implicit arguments*",
    detail: "some detail",
    displayItem: proto.DisplayOption.ImplicitArguments
  };
  export const Coercions: T = {
    label: "Coercions",
    description: "toggle display of *coercions*",
    displayItem: proto.DisplayOption.Coercions
  };
  export const RawMatchingExpressions: T = {
    label: "Raw Matching Expressions",
    description: "toggle display of *raw matching expressions*",
    displayItem: proto.DisplayOption.RawMatchingExpressions
  };
  export const Notations: T = {
    label: "Notations",
    description: "toggle display of notations",
    displayItem: proto.DisplayOption.Notations
  };
  export const AllBasicLowLevelContents: T = {
    label: "All Basic Low Level Contents",
    description: "toggle display of ",
    displayItem: proto.DisplayOption.AllBasicLowLevelContents
  };
  export const ExistentialVariableInstances: T = {
    label: "Existential Variable Instances",
    description: "toggle display of ",
    displayItem: proto.DisplayOption.ExistentialVariableInstances
  };
  export const UniverseLevels: T = {
    label: "Universe Levels",
    description: "toggle display of ",
    displayItem: proto.DisplayOption.UniverseLevels
  };
  export const AllLowLevelContents: T = {
    label: "All Low Level Contents",
    description: "toggle display of ",
    displayItem: proto.DisplayOption.AllLowLevelContents
  };
...
export class CoqDocument implements vscode.Disposable {
  ...
  constructor(document: vscode.TextDocument, project: CoqProject) {
    ...
    if (this.project.settings.showProofViewOn === "open-script") {
      ...
      if (viewCol) this.view.show(adjacentPane(viewCol));
      else this.view.show(vscode.ViewColumn.One);
    }
    ...
    this.langServer.onCoqtopStop(p => {
      if (
        p.reason === proto.CoqtopStopReason.Anomaly ||
        p.reason === proto.CoqtopStopReason.InternalError
      )
        vscode.window.showErrorMessage(
          p.message || "Coqtop quit for an unknown reason."
        );
    ...
    this.subscriptions.push(
      vscode.window.onDidChangeTextEditorSelection(
        (e: vscode.TextEditorSelectionChangeEvent) => {
          if (
            this.project.settings.autoRevealProofStateAtCursor &&
            e.textEditor.document === this.document &&
  ...
  private async refreshGoal(e?: vscode.TextEditor) {
    if (!e) e = vscode.window.activeTextEditor;
    ...
    if (params.routeId == this.queryRouteId) {
      ...
      switch (params.level) {
        case "warning":
          ...
          this.project.infoOut.appendLine(
            psm.prettyTextToString(params.message)
          );
          return;
        case "info":
          this.project.infoOut.appendLine(
        ...
        case "debug":
          ...
          this.project.debugOut.appendLine(
            psm.prettyTextToString(params.message)
          );
          return;
      }
  ...
  private findEditor(): vscode.TextEditor | undefined {
    return vscode.window.visibleTextEditors.find(
      (editor, i, a) => editor.document.uri.toString() === this.documentUri
    );
  }
  ...
  public setCursorToPosition(
    ...
    if (scroll) {
      if (
        scrollHorizontal ||
        textUtil.positionIsBefore(pos, this.document.lineAt(pos.line).range.end)
      )
        editor.revealRange(
        ...
        editor.revealRange(
          new vscode.Range(pos.line, 0, pos.line + 1, 0),
          vscode.TextEditorRevealType.Default
        );
    }
  }
  ...
  private async userSetCoqtopPath(global = false) {
    const current = vscode.workspace
      .getConfiguration("coqtop")
      .get("binPath", "");
    const newPath = await vscode.window.showInputBox({
      ignoreFocusOut: true,
      value: current,
      validateInput: (v: string): string => {
        try {
          const statDir = fs.statSync(v);
          if (!statDir.isDirectory()) return "not a directory";
        } catch (err) {
          return "invalid path";
        }
        ...
        if (!stat) return "coqtop not found here";
        if (!stat.isFile())
    ...
    async function checkCoqtopExists(newPath: string) {
      ...
      try {
        return (
          (await fs.existsSync(path.join(newPath, "coqtop"))) ||
          (await fs.existsSync(path.join(newPath, "coqtop.exe")))
        );
      } catch (err) {
        return false;
      }
    ...
    if (newPath)
      if (await checkCoqtopExists(newPath))
        await vscode.workspace
          .getConfiguration("coqtop")
          .update("binPath", newPath, global);
  }

  private handleResult(value: proto.CommandResult) {
    if (value.type === "busy") return false;
      ...
      if (value.reason === "spawn-failed") {
        ...
        vscode.window
          .showErrorMessage(
            `Could not start coqtop ${
              value.coqtop ? ` (${value.coqtop})` : ""
            }`,
            getCoq,
            setPathLocal,
            setPathGlobal
          ...
          .then(async act => {
            if (act && act.id === getCoq.id) {
              vscode.commands.executeCommand(
                "vscode.open",
                vscode.Uri.parse("https://coq.inria.fr/download")
              );
            } else if (
              act &&
              (act.id === setPathLocal.id || act.id === setPathGlobal.id)
            ) {
    ...
    else this.updateFocus(value.focus, this.project.settings.moveCursorToFocus);
  ...
  private updateView(state: proto.CommandResult, interactive = false) {
    if (
      interactive &&
      !this.view.isVisible() &&
      this.project.settings.showProofViewOn === "first-interaction"
    ) {
      let viewCol = this.currentViewColumn();
      if (viewCol) this.view.show(adjacentPane(viewCol), state);
      else this.view.show(vscode.ViewColumn.One, state);
    } else {
      this.view.update(state);
    }
    this.stateViewFocus =
      state.type === "proof-view"
        ? new vscode.Position(state.focus.line, state.focus.character)
        : undefined;
    this.showFocusDecorations();
  }
  ...
  public async stepBackward(editor: TextEditor) {
    ...
    try {
      ...
      this.updateView(value, true);
      if (this.handleResult(value)) this.statusBar.setStateReady();
  ...
  public async interpretToCursorPosition(
    editor: TextEditor,
    synchronous = false
  ) {
    ...
    try {
      if (!editor || editor.document.uri.toString() !== this.documentUri)
        return;
      const value = await this.langServer.interpretToPoint(
  ...
  public async viewGoalState(editor: TextEditor) {
    try {
      ...
      else await this.view.show(vscode.ViewColumn.One);
    } catch (err) {}
  }
  ...
  public async ltacProfGetResults(editor: TextEditor) {
    ...
    try {
      ...
      this.currentLtacProfView = new HtmlLtacProf({
        total_time: 0,
        tactics: []
      });
  ...
  private onLtacProfResults(results: proto.LtacProfResults) {
    ...
    else this.currentLtacProfView.update(results);
  }
  ...
  private async queryDisplayOptionChange(): Promise<proto.DisplayOption | null> {
    const result = await vscode.window.showQuickPick(
      DisplayOptionPicks.allPicks
    );
    if (result) return result.displayItem;
    else return null;
  }

  public async setDisplayOption(
    item?: proto.DisplayOption,
    value?: proto.SetDisplayOption
  ) {
    if (!item) {
  ...
  public async viewGoalAt(editor: vscode.TextEditor, pos?: vscode.Position) {
    try {
      if (!pos) pos = editor.selection.active;
      const proofview = await this.langServer.getCachedGoal(
        pos,
        this.project.settings.revealProofStateAtCursorDirection
      );
      if (proofview.type === "proof-view") this.updateView(proofview, false);
    } catch (err) {}
  }
